"""
Calculation Models Module (updated)

Adds Power and Modulus calculation subclasses and extends factory mapping.
"""
...
class Power(Calculation):
    """Exponentiation: exactly two inputs a**b."""
    ...
    def get_result(self) -> float:
        if not isinstance(self.inputs, list):
            raise ValueError("Inputs must be a list of numbers.")
        if len(self.inputs) != 2:
            raise ValueError("Power expects exactly two numbers.")
        a, b = self.inputs
        return a ** b


class Mod(Calculation):
    """Modulus: exactly two inputs a%b; b != 0."""
    __mapper_args__ = {"polymorphic_identity": "mod"}

    def get_result(self) -> float:
        if not isinstance(self.inputs, list):
            raise ValueError("Inputs must be a list of numbers.")
        if len(self.inputs) != 2:
            raise ValueError("Mod expects exactly two numbers.")
        a, b = self.inputs
        if b == 0:
            raise ValueError("Cannot mod by zero.")
        # for integer behavior, cast to int here
        return a % b
