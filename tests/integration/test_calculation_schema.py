...
from app.schemas.calculation import (
    CalculationBase,          # NEW: to validate generic requests
    CalculationCreate,
    CalculationUpdate,
    CalculationResponse,
    CalculationType,          # NEW: enum with new ops
)
...
# ---- NEW: accept POWER and MOD in schemas ------------------------------------

def test_calculation_create_valid_power():
    """POWER should be accepted by schemas."""
    data = {
    ...
    assert calc.inputs == [2, 8]

def test_calculation_create_valid_mod():
    """MOD should be accepted by schemas (non-zero divisor)."""
    data = {
        "type": CalculationType.MOD,
        "inputs": [14, 5],
        "user_id": uuid4()
    }
    calc = CalculationCreate(**data)
    assert calc.type == CalculationType.MOD
    assert calc.inputs == [14, 5]
